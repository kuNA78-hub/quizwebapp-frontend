...
const StatCard = ({ value, label, color, textColor = "text-black" }) => (
...
export default function DashboardPage() {
  ...
  const [myRecentResults, setMyRecentResults] = useState([]);
  ...
  if (loading) return (
    <>
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="font-black text-2xl uppercase italic animate-bounce tracking-tighter">
      ...
      <div className="min-h-screen font-sans text-black pb-20">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <div className="flex flex-wrap gap-3 mb-8 justify-center">
            <NeoButton 
              className={`rounded-full px-8 py-3 transition-all ${activeTab === 'overview' ? 'bg-yellow-400 text-black' : 'bg-orange-300 text-black'}`}
            ...
            {isAdmin && (
              <NeoButton 
                className={`rounded-full px-8 py-3 transition-all ${activeTab === 'admin' ? 'bg-purple-500 text-white' : 'bg-purple-300 text-black'}`}
            ...
            <NeoButton 
              className={`rounded-full px-8 py-3 transition-all ${activeTab === 'leaderboard' ? 'bg-yellow-400 text-black' : 'bg-orange-300 text-black'}`}
          ...
          {activeTab === 'overview' && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                {!isAdmin ? (
                  <>
                    <StatCard value={stats.totalQuizzes} label="Quizzes Taken" color="bg-orange-200" />
                    <StatCard value={`${stats.averageScore}%`} label="Avg Score" color="bg-cyan-200" />
                    <StatCard value={stats.passed} label="Passed" color="bg-green-300" />
                  </>
                ) : (
                  <>
                    <StatCard value={adminStats.totalStudents} label="Total Students" color="bg-orange-200" />
                    <StatCard value={adminStats.totalAttempts} label="Quiz Attempts" color="bg-purple-300" />
                    <StatCard value="★" label="Admin Active" color="bg-indigo-300" />
                  </>
                )}
              </div>
              <NeoCard className="rounded-[45px] overflow-hidden p-0 border-4 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] bg-[#f9f9f9]">
                <div className="relative py-12 px-6 bg-[#ebf2ff] border-b-4 border-black overflow-hidden">
                  <div className="absolute -left-2 top-6 bg-yellow-400 border-4 border-black px-8 py-2 rounded-xl -rotate-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-10">
                    <h2 className="text-xl font-black uppercase italic tracking-tighter flex items-center gap-2">
                      ...
                      <span>{!isAdmin ? 'Recent Attempts' : 'Student Activity'}</span>
                    </h2>
                  </div>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                      {(isAdmin ? allResults.slice(0, 10) : myRecentResults).map((attempt) => (
                      ...
                      {((isAdmin ? allResults.length : myRecentResults.length) === 0) && (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    ...
                    {allResults.length === 0 && <tr><td colSpan="5" className="p-8 text-center">No attempts yet.</td></tr>}
                  </tbody>
                </table>
              </div>
            </NeoCard>
            ...
            <NeoCard className="rounded-[45px] overflow-hidden p-0 border-4 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] bg-[#f9f9f9]">
              <div className="relative py-12 px-6 bg-[#ebf2ff] border-b-4 border-black overflow-hidden">
                <div className="absolute -left-2 top-6 bg-green-400 border-4 border-black px-8 py-2 rounded-xl -rotate-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-10">
                  <h2 className="text-xl font-black uppercase italic tracking-tighter flex items-center gap-2">
                    <span className="text-2xl">🏆</span> Student Leaderboard
                  </h2>
                </div>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {leaderboard.map((entry, idx) => (
                      <tr key={entry._id} className={`border-b-4 border-black hover:bg-green-50 transition-colors ${isCurrentUser(entry) ? 'bg-yellow-100' : ''}`}>
                        <td className="p-5 font-black text-center">{idx+1}</td>
                        <td className="p-5 font-black">{entry.username} {isCurrentUser(entry) && <span className="ml-2 text-xs bg-black text-white px-2 py-0.5 rounded-full">You</span>}</td>
                    ...
                    {leaderboard.length === 0 && <tr><td colSpan="5" className="p-8 text-center">No data yet. Students need to take quizzes.</td></tr>}
                  </tbody>
                </table>
              </div>
            </NeoCard>
