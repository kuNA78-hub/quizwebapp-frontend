...
// Playful NavLink with "Sticker" hover effect
const NavLink = ({ to, children, color = "hover:bg-yellow-300" }) => (
  <Link 
    to={to} 
    className={`relative font-black uppercase text-xs tracking-widest px-4 py-2 rounded-xl border-4 border-black transition-all 
      hover:-translate-y-1 hover:-rotate-2 hover:shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] 
      active:translate-y-0 active:rotate-0 active:shadow-none ${color}`}
  >
    {children}
  </Link>
);
        ...
        <div className="flex items-center gap-3">
          ...
          {isAuthenticated ? (
            ...
            <div className="flex gap-2">
              ...
              <Link to="/register" className="bg-black text-white border-4 border-black px-4 py-2 rounded-xl font-black uppercase text-xs hover:bg-[#5c94ff] transition-colors">
                Join
              </Link>
            </div>
          )}
